...
export class SongService {
  ...
  async findSongById(id: number) {
    ...
    try {
      ...
      if (data.length === 0) throw new NotFoundException('Song not found');
      throw new MethodNotAllowedException(
        'Multiple songs found with the same ID',
      );
    } catch (error) {
      if (
  ...
  async getRandomSongs(n: number) {
    if (n < 0 || n > Number.MAX_VALUE) {
      throw new MethodNotAllowedException(
        'number must be between 0 and ' + Number.MAX_VALUE,
      );
    }
    let query: string;
    let param: any[];
  ...
  async deleteSong(id: number) {
    ...
    let deletedSong;
    try {
      deletedSong = await this.databaseService.executeTransaction(
        queryDelete,
        paramsDelete,
      );
    } catch (error) {
  ...
  async findSongsFromPlaylist(playlistId: number) {
    ...
    let param: any[];
    query =
      'SELECT song_id, genre_id,song_name, lyrics,time,song_image,mp3 FROM playlists_songs_view WHERE playlist_id = $1';
    param = [playlistId];
  ...
  async findSongsFromAlbum(albumId: number) {
    ...
    let param: any[];
    query =
      'SELECT song_id, genre_id,song_name, lyrics,time,song_image,mp3 FROM albums_songs_view WHERE album_id = $1';
    param = [albumId];
  ...
  async findSongsFromArtist(artistId: number) {
    ...
    let param: any[];
    query =
      'SELECT song_id, genre_id,song_name, lyrics,time,song_image,mp3 FROM artists_songs_view WHERE artist_id = $1';
    param = [artistId];
    ...
    try {
      ...
      if (error.code === '23503') {
        throw new NotFoundException(
          'Does not exist a song or playlist with these IDs ',
        );
      }
      throw new InternalServerErrorException(
        '(Internal Server Error) -> ' + error,
  ...
  async replaySong(song_id: number) {
    let queryCreate: string;
    let paramsCreate: any[];
    queryCreate = 'INSERT INTO replays (song_id) VALUES ($1) RETURNING *';
  ...
  async addSongToArtist(song_id: number, artistId: number) {
    ...
    try {
      ...
      if (error === '23503') {
        throw new NotFoundException(
          'Does not exist a song or artist with these IDs ',
        );
      }
      throw new InternalServerErrorException(
        '(Internal Server Error) -> ' + error,
  ...
  async deleteSongFromArtist(song_id: number, artistId: number) {
    const queryDelete =
      'DELETE FROM artists_songs WHERE song_id = $1 AND artist_id = $2 RETURNING *';
    ...
    try {
      ...
      if (result.data[0].length === 0) {
        // Suponiendo que result es un array
        throw new NotFoundException(
          'The song or artist does not exist with the given IDs',
        );
      }
  ...
  async deleteSongFromPlaylist(song_id: number, playlistId: number) {
    const queryDelete =
      'DELETE FROM playlists_songs WHERE song_id = $1 AND playlist_id = $2 RETURNING *';
    ...
    try {
      ...
      if (result.data[0].length === 0) {
        // Suponiendo que result es un array
        throw new NotFoundException(
          'The song or playlist does not exist with the given IDs',
        );
      }
  ...
  async addSongToAlbum(song_id: number, albumId: number) {
    ...
    try {
      ...
      if (error === '23503') {
        throw new NotFoundException(
          'Does not exist a song or album with these IDs ',
        );
      }
      throw new InternalServerErrorException(
        '(Internal Server Error) -> ' + error,
  ...
  async deleteSongFromAlbum(song_id: number, albumId: number) {
    const queryDelete =
      'DELETE FROM albums_songs WHERE song_id = $1 AND album_id = $2 RETURNING *';
    ...
    try {
      ...
      if (result.data[0].length === 0) {
        // Suponiendo que result es un array
        throw new NotFoundException(
          'The song or album does not exist with the given IDs',
        );
      }
